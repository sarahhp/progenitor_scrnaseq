...
input_files = snakemake.input.anndata
comparison = snakemake.wildcards.comp
...
fig_path = os.path.join(odir, "figures", comparison)
...
adata_ref = anndata.read_h5ad(input_files[0]) # Reference dataset
adata_query = anndata.read_h5ad(input_files[1]) # Query datase
...
n_bins= int(np.mean([len(optw.splits),len(optb.splits)]))
...
rout = os.path.join(fig_path,"ref")
os.makedirs(rout, exist_ok=True)
for factor in to_plot.keys():
    g2g_barplot(adata_ref, n_bins, factor, to_plot[factor], fig_path=rout)

qout = os.path.join(fig_path,"query")
os.makedirs(qout, exist_ok=True)
for factor in to_plot.keys():
    g2g_barplot(adata_query, n_bins, factor, to_plot[factor], fig_path=qout)
...
if subset_data:
    ...
    adata_ref.obs['bin_ids'].to_csv(os.path.join(odir, "10%_g2g_ref_bin_ids.tsv"), sep='\t')
    adata_query.obs['bin_ids'].to_csv(os.path.join(odir, "10%_g2g_query_bin_ids.tsv"), sep='\t')
elif not subset_data:
   # with open(odir + '/g2g_aligner.pkl', 'wb') as file:
        #pickle.dump(aligner, file)
    df.to_csv(odir + "/g2g_alignment_genes.tsv", sep='\t', index=False)
    adata_ref.obs['bin_ids'].to_csv(os.path.join(odir, "g2g_ref_bin_ids.tsv"), sep='\t')
    adata_query.obs['bin_ids'].to_csv(os.path.join(odir, "g2g_query_bin_ids.tsv"), sep='\t')
